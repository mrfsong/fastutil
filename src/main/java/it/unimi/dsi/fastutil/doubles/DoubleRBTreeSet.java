...
public class DoubleRBTreeSet extends AbstractDoubleSortedSet implements java.io.Serializable, Cloneable, DoubleSortedSet {
	...
	*/
	...
	private final class Subset extends AbstractDoubleSortedSet implements java.io.Serializable, DoubleSortedSet {
	 ...
	 public Subset(final double from, final boolean bottom, final double to, final boolean top) {
	  if (! bottom && ! top && DoubleRBTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
	 ...
	 private final class SubsetIterator extends SetIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && DoubleRBTreeSet.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && DoubleRBTreeSet.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
	}
