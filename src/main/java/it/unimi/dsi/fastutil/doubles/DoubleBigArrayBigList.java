...
public class DoubleBigArrayBigList extends AbstractDoubleBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	protected DoubleBigArrayBigList(final double a[][], @SuppressWarnings("unused") boolean dummy) {
	 this.a = a;
	...
	public DoubleBigArrayBigList(final long capacity) {
	 ...
	 if (capacity == 0) a = DoubleBigArrays.EMPTY_BIG_ARRAY;
	 else a = DoubleBigArrays.newBigArray(capacity);
	...
	public void ensureCapacity(final long capacity) {
	 if (capacity <= a.length || a == DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;

	 a = DoubleBigArrays.forceCapacity(a, capacity, size);
	...
	private void grow(long capacity) {
	 final long oldLength = DoubleBigArrays.length(a);
	 if (capacity <= oldLength) return;
	 if (a != DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY) capacity = Math.max(oldLength + (oldLength >> 1), capacity);
	...
	public long indexOf(final double k) {
	 for(long i = 0; i < size; i++) if (( Double.doubleToLongBits(k) == Double.doubleToLongBits(DoubleBigArrays.get(a, i)) )) return i;
	 return -1;
	}

	@Override
	public long lastIndexOf(final double k) {
	 for(long i = size; i-- != 0;) if (( Double.doubleToLongBits(k) == Double.doubleToLongBits(DoubleBigArrays.get(a, i)) )) return i;
	 return -1;
	}
	...
	public void size(final long size) {
	 if (size > DoubleBigArrays.length(a)) a = DoubleBigArrays.forceCapacity(a, size, this.size);
	 if (size > this.size) DoubleBigArrays.fill(a, this.size, size, (0));
