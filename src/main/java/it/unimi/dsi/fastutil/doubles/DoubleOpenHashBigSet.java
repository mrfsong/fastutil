...
public class DoubleOpenHashBigSet extends AbstractDoubleSet implements java.io.Serializable, Cloneable, Hash, Size64 {
	...
	public boolean add(final double k) {
	 ...
	 if (( Double.doubleToLongBits(k) == 0 )) {
	  ...
	  if (! ( Double.doubleToLongBits(curr = key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 )) {
	   ...
	   while(! ( Double.doubleToLongBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 ))
	    if (( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) )) return false;
	  }
	...
	protected final void shiftKeys(long pos) {
	 ...
	 for(;;) {
	  ...
	  for(;;) {
	   if (( Double.doubleToLongBits(DoubleBigArrays.get(key, pos)) == 0 )) {
	    DoubleBigArrays.set(key, last, (0));
	    return;
	   }
	   slot = it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(DoubleBigArrays.get(key, pos)) ) & mask;
	...
	private class SetIterator implements DoubleIterator {
	 ...
	 private final void shiftKeys(long pos) {
	  ...
	  for(;;) {
	   ...
	   for(;;) {
	    if(( Double.doubleToLongBits(curr = DoubleBigArrays.get(key, pos)) == 0 )) {
	     DoubleBigArrays.set(key, last, (0));
	     return;
	    }
	...
	protected void rehash(final long newN) {
	 ...
	 for(long i = realSize(); i-- != 0;) {
	  ...
	  // The starting point.
	  if (! ( Double.doubleToLongBits(newKey[b = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][d = (int)(h & newSegmentMask)]) == 0 ))
	   while(! ( Double.doubleToLongBits(newKey[b = (b + ((d = (d + 1) & newSegmentMask) == 0 ? 1 : 0)) & newBaseMask][d]) == 0 ));
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
	 ...
	 for(long i = size; i-- != 0;) {
	  ...
	  if (( Double.doubleToLongBits(k) == 0 )) containsNull = true;
	   ...
	   h = it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) );
	   if (! ( Double.doubleToLongBits(key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 ))
	    while(! ( Double.doubleToLongBits(key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 ));
	   key[base][displ] = k;
	  }
	 }
