...
public class ObjectHeapSemiIndirectPriorityQueue <K> implements IndirectPriorityQueue <K> {
	 ...
	 * @param c the comparator used in this queue, or {@code null} for the natural order.
	 */
	public ObjectHeapSemiIndirectPriorityQueue(final K[] refArray, final int[] a, int size, final Comparator <? super K> c) {
	 this(refArray, 0, c);
	 this.heap = a;
	 this.size = size;
	...
	public int dequeue() {
	 ...
	 heap[0] = heap[--size];
	 if (size != 0) ObjectSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
	 return result;
	}
	...
	@Override
	public void allChanged() { ObjectSemiIndirectHeaps.makeHeap(refArray, heap, size, c); }
	 ...
	 */
	@Override
	public int front(final int[] a) { return c == null ? ObjectSemiIndirectHeaps.front(refArray, heap, size, a) : ObjectSemiIndirectHeaps.front(refArray, heap, size, a, c); }
