...
public class ReferenceBigArrayBigList <K> extends AbstractReferenceBigList <K> implements RandomAccess, Cloneable, java.io.Serializable {
	...
	protected ReferenceBigArrayBigList(final K a[][], @SuppressWarnings("unused") boolean dummy) {
	 this.a = a;
	...
	public void ensureCapacity(final long capacity) {
	 if (capacity <= a.length || a == ObjectBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;



	 if (wrapped) a = ObjectBigArrays.forceCapacity(a, capacity, size);
	 else {
	 ...
	 assert size <= ObjectBigArrays.length(a);
	}
	...
	private void grow(long capacity) {
	 final long oldLength = ObjectBigArrays.length(a);
	...
	public long indexOf(final Object k) {
	 for(long i = 0; i < size; i++) if (( (k) == (ObjectBigArrays.get(a, i)) )) return i;
	 return -1;
	}

	@Override
	public long lastIndexOf(final Object k) {
	 for(long i = size; i-- != 0;) if (( (k) == (ObjectBigArrays.get(a, i)) )) return i;
	 return -1;
	}
	...
	public boolean removeAll(final Collection<?> c) {
	 ...
	 ObjectBigArrays.fill(a, j, size, null);
	...
	public void size(final long size) {
	 if (size > ObjectBigArrays.length(a)) a = ObjectBigArrays.forceCapacity(a, size, this.size);
	...
	public boolean equals(final ReferenceBigArrayBigList <K> l) {
	 ...
	 while(s-- != 0) if (ObjectBigArrays.get(a1, s) != ObjectBigArrays.get(a2, s)) return false;
