...
public class ObjectAVLTreeSet <K> extends AbstractObjectSortedSet <K> implements java.io.Serializable, Cloneable, ObjectSortedSet <K> {
	...
	*/
	...
	@Override
	public ObjectBidirectionalIterator <K> iterator() { return new SetIterator(); }

	@Override
	public ObjectBidirectionalIterator <K> iterator(final K from) { return new SetIterator(from); }

	@Override
	public Comparator <? super K> comparator() { return actualComparator; }

	@Override
	public ObjectSortedSet <K> headSet(final K to) { return new Subset((null), true, to, false); }

	@Override
	public ObjectSortedSet <K> tailSet(final K from) { return new Subset(from, false, (null), true); }

	@Override
	public ObjectSortedSet <K> subSet(final K from, final K to) { return new Subset(from, false, to, false); }
	...
	private final class Subset extends AbstractObjectSortedSet <K> implements java.io.Serializable, ObjectSortedSet <K> {
	 ...
	 public Subset(final K from, final boolean bottom, final K to, final boolean top) {
	  if (! bottom && ! top && ObjectAVLTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
	 ...
	 @Override
	 public ObjectBidirectionalIterator <K> iterator() { return new SubsetIterator(); }

	 @Override
	 public ObjectBidirectionalIterator <K> iterator(final K from) { return new SubsetIterator(from); }
	 ...
	 private final class SubsetIterator extends SetIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && ObjectAVLTreeSet.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && ObjectAVLTreeSet.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
	}
