...
public abstract class AbstractReferenceBigList <K> extends AbstractReferenceCollection <K> implements ReferenceBigList <K>, Stack <K> {
	...
	public ObjectBigListIterator <K> listIterator(final long index) {
	 ...
	 return new ObjectBigListIterator <K>() {
	   ...
	   @Override
	   public boolean hasNext() { return pos < AbstractReferenceBigList.this.size64(); }

	   @Override
	   public boolean hasPrevious() { return pos > 0; }

	   @Override
	   public K next() { if (! hasNext()) throw new NoSuchElementException(); return AbstractReferenceBigList.this.get(last = pos++); }

	   @Override
	   public K previous() { if (! hasPrevious()) throw new NoSuchElementException(); return AbstractReferenceBigList.this.get(last = --pos); }
