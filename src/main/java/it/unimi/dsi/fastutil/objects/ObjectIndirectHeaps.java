	...
	public static <K> int downHeap(final K[] refArray, final int[] heap, final int[] inv, final int size, int i, final Comparator <K> c) {
	 ...
	 if (c == null)
	  while ((child = (i << 1) + 1) < size) {
	   ...
	   if (right < size && ( ((Comparable<K>)(refArray[heap[right]])).compareTo(refArray[t]) < 0 )) t = heap[child = right];
	   if (( ((Comparable<K>)(E)).compareTo(refArray[t]) <= 0 )) break;
	   heap[i] = t;
	   inv[heap[i]] = i;
	   i = child;
	  }
	...
	public static <K> int upHeap(final K[] refArray, final int[] heap, final int[] inv, final int size, int i, final Comparator <K> c) {
	 ...
	 if (c == null)
	  while (i != 0) {
	   ...
	   if (( ((Comparable<K>)(refArray[t])).compareTo(E) <= 0 )) break;
	   heap[i] = t;
	   inv[heap[i]] = i;
	   i = parent;
	  }
