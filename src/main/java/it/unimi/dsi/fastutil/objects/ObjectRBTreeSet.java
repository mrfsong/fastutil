...
public class ObjectRBTreeSet <K> extends AbstractObjectSortedSet <K> implements java.io.Serializable, Cloneable, ObjectSortedSet <K> {
	...
	*/
	...
	private final class Subset extends AbstractObjectSortedSet <K> implements java.io.Serializable, ObjectSortedSet <K> {
	 ...
	 public Subset(final K from, final boolean bottom, final K to, final boolean top) {
	  if (! bottom && ! top && ObjectRBTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
	 ...
	 private final class SubsetIterator extends SetIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && ObjectRBTreeSet.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && ObjectRBTreeSet.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
	}
