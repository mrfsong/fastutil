...
public final class ObjectBigArrays {
	...
	public static <K> K[][] wrap(final K[] array) {
	 if (array.length == 0 && array.getClass() == Object[].class) return (K[][]) EMPTY_BIG_ARRAY;
	...
	private static final class BigArrayHashStrategy <K> implements Hash.Strategy<K[][]>, java.io.Serializable {
	 ...
	 @Override
	 public int hashCode(final K[][] o) { return java.util.Arrays.deepHashCode(o); }

	 @Override
	 public boolean equals(final K[][] a, final K[][] b) { return ObjectBigArrays.equals(a, b); }
	}
	...
	private static <K> void selectionSort(final K[][] a, final long from, final long to, final Comparator <K> comp) {
	 for(long i = from; i < to - 1; i++) {
	  long m = i;
	  for(long j = i + 1; j < to; j++) if (comp.compare(ObjectBigArrays.get(a, j), ObjectBigArrays.get(a, m)) < 0) m = j;
	  if (m != i) swap(a, i, m);
	 }
	}
	...
	private static <K> long med3(final K x[][], final long a, final long b, final long c) {
	 int ab = ( ((Comparable<K>)(get(x, a))).compareTo(get(x, b)) );
	 int ac = ( ((Comparable<K>)(get(x, a))).compareTo(get(x, c)) );
	 int bc = ( ((Comparable<K>)(get(x, b))).compareTo(get(x, c)) );
	 return (ab < 0 ?
	  (bc < 0 ? b : ac < 0 ? c : a) :
	  (bc > 0 ? b : ac > 0 ? c : a));
	}
	...
	private static <K> void selectionSort(final K[][] a, final long from, final long to) {
	 for(long i = from; i < to - 1; i++) {
	  long m = i;
	  for(long j = i + 1; j < to; j++) if (( ((Comparable<K>)(ObjectBigArrays.get(a, j))).compareTo(ObjectBigArrays.get(a, m)) < 0 )) m = j;
	  if (m != i) swap(a, i, m);
	 }
	}
	...
	public static <K> void quickSort(final K[][] x, final long from, final long to) {
	 ...
	 while(true) {
	  int comparison;
	  while (b <= c && (comparison = ( ((Comparable<K>)(get(x, b))).compareTo(v) )) <= 0) {
	   if (comparison == 0) swap(x, a++, b);
	   b++;
	  }
	  while (c >= b && (comparison = ( ((Comparable<K>)(get(x, c))).compareTo(v) )) >=0) {
	   if (comparison == 0) swap(x, c, d--);
	   c--;
	  }
	  if (b > c) break;
