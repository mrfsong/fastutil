...
public class Short2LongRBTreeMap extends AbstractShort2LongSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Short2LongMap.Entry > short2LongEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Short2LongMap.Entry >() {
	   final Comparator<? super Short2LongMap.Entry > comparator = (Comparator<Short2LongMap.Entry >) (x, y) -> Short2LongRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	   ...
	   @Override
	   public void clear() { Short2LongRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractShort2LongSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final short from, final boolean bottom, final short to, final boolean top) {
	  if (! bottom && ! top && Short2LongRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Short2LongMap.Entry > short2LongEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Short2LongMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Short2LongMap.Entry > comparator() { return Short2LongRBTreeMap.this.short2LongEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final short k) { return in( k) && Short2LongRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Short2LongRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Short2LongRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
