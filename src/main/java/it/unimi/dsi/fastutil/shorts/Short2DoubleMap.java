...
public interface Short2DoubleMap extends Short2DoubleFunction , Map<Short,Double> {
	...
	default double replace(final short key, final double value) { return containsKey(key) ? put(key, value) : defaultReturnValue(); }
	...
	default double computeIfAbsentNullable(final short key, final java.util.function.IntFunction<? extends Double> mappingFunction) {
	 java.util.Objects.requireNonNull(mappingFunction);
	 final double v = get(key), drv = defaultReturnValue();
	 if (v != drv || containsKey(key)) return v;
	...
	default double computeIfPresent(final short key, final java.util.function.BiFunction<? super Short, ? super Double, ? extends Double> remappingFunction) {
	...
	default double compute(final short key, final java.util.function.BiFunction<? super Short, ? super Double, ? extends Double> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final double oldValue = get(key), drv = defaultReturnValue();
	 final boolean contained = oldValue != drv || containsKey(key);
	 final Double newValue = remappingFunction.apply(Short.valueOf(key), contained ? Double.valueOf(oldValue) : null);
	 ...
	 * @since 8.0.0
	 */
	default double merge(final short key, final double value, final java.util.function.BiFunction<? super Double, ? super Double, ? extends Double> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
