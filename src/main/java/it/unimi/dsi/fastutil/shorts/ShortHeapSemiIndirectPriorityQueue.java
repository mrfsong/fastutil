...
public class ShortHeapSemiIndirectPriorityQueue implements ShortIndirectPriorityQueue {
	 ...
	 * @param c the comparator used in this queue, or {@code null} for the natural order.
	 */
	public ShortHeapSemiIndirectPriorityQueue(final short[] refArray, final int[] a, int size, final ShortComparator c) {
	 this(refArray, 0, c);
	 this.heap = a;
	 this.size = size;
	...
	public int dequeue() {
	 ...
	 heap[0] = heap[--size];
	 if (size != 0) ShortSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
	 return result;
	}
	...
	@Override
	public void allChanged() { ShortSemiIndirectHeaps.makeHeap(refArray, heap, size, c); }
	 ...
	 */
	@Override
	public int front(final int[] a) { return c == null ? ShortSemiIndirectHeaps.front(refArray, heap, size, a) : ShortSemiIndirectHeaps.front(refArray, heap, size, a, c); }
