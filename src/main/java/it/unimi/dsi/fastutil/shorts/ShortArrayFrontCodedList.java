...
public class ShortArrayFrontCodedList extends AbstractObjectList<short[]> implements Serializable, Cloneable, RandomAccess {
	...
	private int extract(final int index, final short a[], final int offset, final int length) {
	 ...
	 if (delta == 0) {
	  ...
	  ShortBigArrays.copyFromBig(array, pos, a, offset, Math.min(length, arrayLength));
	  return arrayLength;
	 }
	 ...
	 for(int i = 0; i < delta; i++) {
	  ...
	  if (actualCommon <= currLen) currLen = actualCommon;
	   ...
	   ShortBigArrays.copyFromBig(array, prevArrayPos, a, currLen + offset, actualCommon - currLen);
	   currLen = actualCommon;
	  }
	 }

	 if (currLen < length) ShortBigArrays.copyFromBig(array, pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
	 ...
	 return new ObjectListIterator<short[]>() {
	   ...
	   public short[] next() {
	    ...
	    if (i % ratio == 0) {
	     ...
	     ShortBigArrays.copyFromBig(array, pos + count(length), s, 0, length);
	     ...
	     if (inSync) {
	      ...
	      ShortBigArrays.copyFromBig(array, pos + count(length) + count (common), s, common, length);
	      pos += count(length) + count(common) + length;
	      length += common;
	     }
	     else {
