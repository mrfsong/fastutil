...
public class Short2ShortRBTreeMap extends AbstractShort2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Short2ShortMap.Entry > short2ShortEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Short2ShortMap.Entry >() {
	   final Comparator<? super Short2ShortMap.Entry > comparator = (Comparator<Short2ShortMap.Entry >) (x, y) -> Short2ShortRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	   ...
	   @Override
	   public void clear() { Short2ShortRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractShort2ShortSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final short from, final boolean bottom, final short to, final boolean top) {
	  if (! bottom && ! top && Short2ShortRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Short2ShortMap.Entry > short2ShortEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Short2ShortMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Short2ShortMap.Entry > comparator() { return Short2ShortRBTreeMap.this.short2ShortEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final short k) { return in( k) && Short2ShortRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Short2ShortRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Short2ShortRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
