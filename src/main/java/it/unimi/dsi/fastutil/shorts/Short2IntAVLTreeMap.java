...
public class Short2IntAVLTreeMap extends AbstractShort2IntSortedMap implements java.io.Serializable, Cloneable {
	...
	*/
	...
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Short2IntMap.Entry > {
	 ...
	 @Override
	 public Short2IntMap.Entry next() { return nextEntry(); }

	 @Override
	 public Short2IntMap.Entry previous() { return previousEntry(); }

	 @Override
	 public void set(Short2IntMap.Entry ok) { throw new UnsupportedOperationException(); }

	 @Override
	 public void add(Short2IntMap.Entry ok) { throw new UnsupportedOperationException(); }
	}


	@Override
	public ObjectSortedSet<Short2IntMap.Entry > short2IntEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Short2IntMap.Entry >() {
	   final Comparator<? super Short2IntMap.Entry > comparator = (Comparator<Short2IntMap.Entry >) (x, y) -> Short2IntAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());

	   @Override
	   public Comparator<? super Short2IntMap.Entry > comparator() { return comparator; }

	   @Override
	   public ObjectBidirectionalIterator<Short2IntMap.Entry > iterator() { return new EntryIterator(); }

	   @Override
	   public ObjectBidirectionalIterator<Short2IntMap.Entry > iterator(final Short2IntMap.Entry from) { return new EntryIterator(from.getShortKey()); }
	   ...
	   @Override
	   public void clear() { Short2IntAVLTreeMap.this.clear(); }

	   @Override
	   public Short2IntMap.Entry first() { return firstEntry; }

	   @Override
	   public Short2IntMap.Entry last() { return lastEntry; }

	   @Override
	   public ObjectSortedSet<Short2IntMap.Entry > subSet(Short2IntMap.Entry from, Short2IntMap.Entry to) { return subMap(from.getShortKey(), to.getShortKey()).short2IntEntrySet(); }

	   @Override
	   public ObjectSortedSet<Short2IntMap.Entry > headSet(Short2IntMap.Entry to) { return headMap(to.getShortKey()).short2IntEntrySet(); }

	   @Override
	   public ObjectSortedSet<Short2IntMap.Entry > tailSet(Short2IntMap.Entry from) { return tailMap(from.getShortKey()).short2IntEntrySet(); }
	  };
	...
	@Override
	public Short2IntSortedMap headMap(short to) { return new Submap(((short)0), true, to, false); }

	@Override
	public Short2IntSortedMap tailMap(short from) { return new Submap(from, false, ((short)0), true); }

	@Override
	public Short2IntSortedMap subMap(short from, short to) { return new Submap(from, false, to, false); }
	...
	private final class Submap extends AbstractShort2IntSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final short from, final boolean bottom, final short to, final boolean top) {
	  if (! bottom && ! top && Short2IntAVLTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Short2IntMap.Entry > short2IntEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Short2IntMap.Entry >() {
	    @Override
	    public ObjectBidirectionalIterator<Short2IntMap.Entry > iterator() { return new SubmapEntryIterator(); }
	    @Override
	    public ObjectBidirectionalIterator<Short2IntMap.Entry > iterator(final Short2IntMap.Entry from) { return new SubmapEntryIterator(from.getShortKey()); }
	    @Override
	    public Comparator<? super Short2IntMap.Entry > comparator() { return Short2IntAVLTreeMap.this.short2IntEntrySet().comparator(); }
	    @Override
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Short2IntAVLTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Short2IntAVLTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
