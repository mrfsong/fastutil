...
public class ShortBigArrayBigList extends AbstractShortBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	protected ShortBigArrayBigList(final short a[][], @SuppressWarnings("unused") boolean dummy) {
	 this.a = a;
	...
	public ShortBigArrayBigList(final long capacity) {
	 ...
	 if (capacity == 0) a = ShortBigArrays.EMPTY_BIG_ARRAY;
	 else a = ShortBigArrays.newBigArray(capacity);
	...
	public ShortBigArrayBigList(final ShortCollection c) {
	 this(c.size());
	 for(ShortIterator i = c.iterator(); i.hasNext();) add(i.nextShort());
	}
	...
	public ShortBigArrayBigList(final short a[][]) {
	 this(a, 0, ShortBigArrays.length(a));
	}
	...
	public ShortBigArrayBigList(final short a[][], final long offset, final long length) {
	 this(length);
	 ShortBigArrays.copy(a, offset, this.a, 0, length);
	 size = length;
	}
	...
	public static ShortBigArrayBigList wrap(final short a[][], final long length) {
	 if (length > ShortBigArrays.length(a)) throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size (" + ShortBigArrays.length(a) + ")");
	...
	public void ensureCapacity(final long capacity) {
	 if (capacity <= a.length || a == ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;

	 a = ShortBigArrays.forceCapacity(a, capacity, size);
	 assert size <= ShortBigArrays.length(a);
	}
	...
	private void grow(long capacity) {
	 final long oldLength = ShortBigArrays.length(a);
	 if (capacity <= oldLength) return;
	 if (a != ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY) capacity = Math.max(oldLength + (oldLength >> 1), capacity);
	...
	public void add(final long index, final short k) {
	 ensureIndex(index);
	 grow(size + 1);
	 if (index != size) ShortBigArrays.copy(a, index, a, index + 1, size - index);
	 ShortBigArrays.set(a, index, k);
	 size++;
	 assert size <= ShortBigArrays.length(a);
	}
	...
	public long indexOf(final short k) {
	 for(long i = 0; i < size; i++) if (( (k) == (ShortBigArrays.get(a, i)) )) return i;
	 return -1;
	}

	@Override
	public long lastIndexOf(final short k) {
	 for(long i = size; i-- != 0;) if (( (k) == (ShortBigArrays.get(a, i)) )) return i;
	 return -1;
	}
	...
	public short removeShort(final long index) {
	 if (index >= size) throw new IndexOutOfBoundsException("Index (" + index + ") is greater than or equal to list size (" + size + ")");
	 final short old = ShortBigArrays.get(a, index);
	 size--;
	 if (index != size) ShortBigArrays.copy(a, index + 1, a, index, size - index);
	...
	public short set(final long index, final short k) {
	 if (index >= size) throw new IndexOutOfBoundsException("Index (" + index + ") is greater than or equal to list size (" + size + ")");
	 short old = ShortBigArrays.get(a, index);
	 ShortBigArrays.set(a, index, k);
	 return old;
	}
	...
	public void size(final long size) {
	 if (size > ShortBigArrays.length(a)) a = ShortBigArrays.forceCapacity(a, size, this.size);
	 if (size > this.size) ShortBigArrays.fill(a, this.size, size, ((short)0));
	...
	public void trim(final long n) {
	 final long arrayLength = ShortBigArrays.length(a);
	 if (n >= arrayLength || size == arrayLength) return;
	 a = ShortBigArrays.trim(a, Math.max(n, size));
	 assert size <= ShortBigArrays.length(a);
	}
	...
	public void removeElements(final long from, final long to) {
	 ...
	 ShortBigArrays.copy(a, to, a, from, size - to);
	 size -= (to - from);
	...
	public void addElements(final long index, final short a[][], final long offset, final long length) {
	 ...
	 grow(size + length);
	 ShortBigArrays.copy(this.a, index, this.a, index + length, size - index);
	...
	public ShortBigListIterator listIterator(final long index) {
	 ...
	 return new ShortBigListIterator () {
	   ...
	   @Override
	   public short nextShort() { if (! hasNext()) throw new NoSuchElementException(); return ShortBigArrays.get(a, last = pos++); }
	   @Override
	   public short previousShort() { if (! hasPrevious()) throw new NoSuchElementException(); return ShortBigArrays.get(a, last = --pos); }
	   @Override
	...
	public ShortBigArrayBigList clone() {
	 ...
	 ShortBigArrays.copy(a, 0, c.a, 0, size);
	 c.size = size;
	 return c;
	}
	...
	public boolean equals(final ShortBigArrayBigList l) {
	 ...
	 while(s-- != 0) if (ShortBigArrays.get(a1, s) != ShortBigArrays.get(a2, s)) return false;
	...
	public int compareTo(final ShortBigArrayBigList l) {
	 ...
	 for(i = 0; i < s1 && i < s2; i++) {
	  e1 = ShortBigArrays.get(a1, i);
	  e2 = ShortBigArrays.get(a2, i);
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
	 ...
	 for(int i = 0; i < size; i++) s.writeShort(ShortBigArrays.get(a, i));
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
	 ...
	 for(int i = 0; i < size; i++) ShortBigArrays.set(a, i, s.readShort());
	}
}
