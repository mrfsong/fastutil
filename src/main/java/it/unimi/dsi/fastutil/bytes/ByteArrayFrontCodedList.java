...
public class ByteArrayFrontCodedList extends AbstractObjectList<byte[]> implements Serializable, Cloneable, RandomAccess {
	...
	private int extract(final int index, final byte a[], final int offset, final int length) {
	 ...
	 if (delta == 0) {
	  ...
	  ByteBigArrays.copyFromBig(array, pos, a, offset, Math.min(length, arrayLength));
	  return arrayLength;
	 }
	 ...
	 for(int i = 0; i < delta; i++) {
	  ...
	  if (actualCommon <= currLen) currLen = actualCommon;
	   ...
	   ByteBigArrays.copyFromBig(array, prevArrayPos, a, currLen + offset, actualCommon - currLen);
	   currLen = actualCommon;
	  }
	 }

	 if (currLen < length) ByteBigArrays.copyFromBig(array, pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
	 ...
	 return new ObjectListIterator<byte[]>() {
	   ...
	   public byte[] next() {
	    ...
	    if (i % ratio == 0) {
	     ...
	     ByteBigArrays.copyFromBig(array, pos + count(length), s, 0, length);
	     ...
	     if (inSync) {
	      ...
	      ByteBigArrays.copyFromBig(array, pos + count(length) + count (common), s, common, length);
	      pos += count(length) + count(common) + length;
	      length += common;
	     }
	     else {
