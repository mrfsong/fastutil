...
public class CharArrayFrontCodedList extends AbstractObjectList<char[]> implements Serializable, Cloneable, RandomAccess {
	...
	private int extract(final int index, final char a[], final int offset, final int length) {
	 ...
	 if (currLen < length) CharBigArrays.copyFromBig(array, pos + count(arrayLength) + count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
	 ...
	 return new ObjectListIterator<char[]>() {
	   ...
	   public char[] next() {
	    ...
	    if (i % ratio == 0) {
	     ...
	     CharBigArrays.copyFromBig(array, pos + count(length), s, 0, length);
	     ...
	     if (inSync) {
	      ...
	      CharBigArrays.copyFromBig(array, pos + count(length) + count (common), s, common, length);
	      pos += count(length) + count(common) + length;
	      length += common;
	     }
	     else {
