...
public class CharBigArrayBigList extends AbstractCharBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public void ensureCapacity(final long capacity) {
	 if (capacity <= a.length || a == CharBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;

	 a = CharBigArrays.forceCapacity(a, capacity, size);
	 assert size <= CharBigArrays.length(a);
	}
	...
	private void grow(long capacity) {
	 final long oldLength = CharBigArrays.length(a);
	...
	public void size(final long size) {
	 if (size > CharBigArrays.length(a)) a = CharBigArrays.forceCapacity(a, size, this.size);
