...
public class Long2IntRBTreeMap extends AbstractLong2IntSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Long2IntMap.Entry > long2IntEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Long2IntMap.Entry >() {
	   final Comparator<? super Long2IntMap.Entry > comparator = (Comparator<Long2IntMap.Entry >) (x, y) -> Long2IntRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	   ...
	   @Override
	   public void clear() { Long2IntRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractLong2IntSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final long from, final boolean bottom, final long to, final boolean top) {
	  if (! bottom && ! top && Long2IntRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Long2IntMap.Entry > long2IntEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Long2IntMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Long2IntMap.Entry > comparator() { return Long2IntRBTreeMap.this.long2IntEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final long k) { return in( k) && Long2IntRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Long2IntRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Long2IntRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
