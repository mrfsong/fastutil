...
public class LongAVLTreeSet extends AbstractLongSortedSet implements java.io.Serializable, Cloneable, LongSortedSet {
	...
	*/
	...
	@Override
	public LongBidirectionalIterator iterator() { return new SetIterator(); }

	@Override
	public LongBidirectionalIterator iterator(final long from) { return new SetIterator(from); }

	@Override
	public LongComparator comparator() { return actualComparator; }

	@Override
	public LongSortedSet headSet(final long to) { return new Subset((0), true, to, false); }

	@Override
	public LongSortedSet tailSet(final long from) { return new Subset(from, false, (0), true); }

	@Override
	public LongSortedSet subSet(final long from, final long to) { return new Subset(from, false, to, false); }
	...
	private final class Subset extends AbstractLongSortedSet implements java.io.Serializable, LongSortedSet {
	 ...
	 public Subset(final long from, final boolean bottom, final long to, final boolean top) {
	  if (! bottom && ! top && LongAVLTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
	 ...
	 @Override
	 public LongBidirectionalIterator iterator() { return new SubsetIterator(); }

	 @Override
	 public LongBidirectionalIterator iterator(final long from) { return new SubsetIterator(from); }
	 ...
	 private final class SubsetIterator extends SetIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && LongAVLTreeSet.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && LongAVLTreeSet.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
	}
