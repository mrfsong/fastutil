...
public class Long2ReferenceRBTreeMap <V> extends AbstractLong2ReferenceSortedMap <V> implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Long2ReferenceMap.Entry <V> > long2ReferenceEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Long2ReferenceMap.Entry <V> >() {
	   final Comparator<? super Long2ReferenceMap.Entry <V> > comparator = (Comparator<Long2ReferenceMap.Entry <V> >) (x, y) -> Long2ReferenceRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	   ...
	   @Override
	   public void clear() { Long2ReferenceRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractLong2ReferenceSortedMap <V> implements java.io.Serializable {
	 ...
	 public Submap(final long from, final boolean bottom, final long to, final boolean top) {
	  if (! bottom && ! top && Long2ReferenceRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Long2ReferenceMap.Entry <V> > long2ReferenceEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Long2ReferenceMap.Entry <V> >() {
	    ...
	    @Override
	    public Comparator<? super Long2ReferenceMap.Entry <V> > comparator() { return Long2ReferenceRBTreeMap.this.long2ReferenceEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final long k) { return in( k) && Long2ReferenceRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Long2ReferenceRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Long2ReferenceRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
