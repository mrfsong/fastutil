...
public class Long2ShortAVLTreeMap extends AbstractLong2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	*/
	...
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Long2ShortMap.Entry > {
	 ...
	 @Override
	 public Long2ShortMap.Entry next() { return nextEntry(); }

	 @Override
	 public Long2ShortMap.Entry previous() { return previousEntry(); }

	 @Override
	 public void set(Long2ShortMap.Entry ok) { throw new UnsupportedOperationException(); }

	 @Override
	 public void add(Long2ShortMap.Entry ok) { throw new UnsupportedOperationException(); }
	}


	@Override
	public ObjectSortedSet<Long2ShortMap.Entry > long2ShortEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Long2ShortMap.Entry >() {
	   final Comparator<? super Long2ShortMap.Entry > comparator = (Comparator<Long2ShortMap.Entry >) (x, y) -> Long2ShortAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());

	   @Override
	   public Comparator<? super Long2ShortMap.Entry > comparator() { return comparator; }

	   @Override
	   public ObjectBidirectionalIterator<Long2ShortMap.Entry > iterator() { return new EntryIterator(); }

	   @Override
	   public ObjectBidirectionalIterator<Long2ShortMap.Entry > iterator(final Long2ShortMap.Entry from) { return new EntryIterator(from.getLongKey()); }
	   ...
	   @Override
	   public void clear() { Long2ShortAVLTreeMap.this.clear(); }

	   @Override
	   public Long2ShortMap.Entry first() { return firstEntry; }

	   @Override
	   public Long2ShortMap.Entry last() { return lastEntry; }

	   @Override
	   public ObjectSortedSet<Long2ShortMap.Entry > subSet(Long2ShortMap.Entry from, Long2ShortMap.Entry to) { return subMap(from.getLongKey(), to.getLongKey()).long2ShortEntrySet(); }

	   @Override
	   public ObjectSortedSet<Long2ShortMap.Entry > headSet(Long2ShortMap.Entry to) { return headMap(to.getLongKey()).long2ShortEntrySet(); }

	   @Override
	   public ObjectSortedSet<Long2ShortMap.Entry > tailSet(Long2ShortMap.Entry from) { return tailMap(from.getLongKey()).long2ShortEntrySet(); }
	  };
	...
	@Override
	public Long2ShortSortedMap headMap(long to) { return new Submap((0), true, to, false); }

	@Override
	public Long2ShortSortedMap tailMap(long from) { return new Submap(from, false, (0), true); }

	@Override
	public Long2ShortSortedMap subMap(long from, long to) { return new Submap(from, false, to, false); }
	...
	private final class Submap extends AbstractLong2ShortSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final long from, final boolean bottom, final long to, final boolean top) {
	  if (! bottom && ! top && Long2ShortAVLTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Long2ShortMap.Entry > long2ShortEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Long2ShortMap.Entry >() {
	    @Override
	    public ObjectBidirectionalIterator<Long2ShortMap.Entry > iterator() { return new SubmapEntryIterator(); }
	    @Override
	    public ObjectBidirectionalIterator<Long2ShortMap.Entry > iterator(final Long2ShortMap.Entry from) { return new SubmapEntryIterator(from.getLongKey()); }
	    @Override
	    public Comparator<? super Long2ShortMap.Entry > comparator() { return Long2ShortAVLTreeMap.this.long2ShortEntrySet().comparator(); }
	    @Override
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Long2ShortAVLTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Long2ShortAVLTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
