...
public class Long2ShortRBTreeMap extends AbstractLong2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Long2ShortMap.Entry > long2ShortEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Long2ShortMap.Entry >() {
	   final Comparator<? super Long2ShortMap.Entry > comparator = (Comparator<Long2ShortMap.Entry >) (x, y) -> Long2ShortRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	   ...
	   @Override
	   public void clear() { Long2ShortRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractLong2ShortSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final long from, final boolean bottom, final long to, final boolean top) {
	  if (! bottom && ! top && Long2ShortRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Long2ShortMap.Entry > long2ShortEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Long2ShortMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Long2ShortMap.Entry > comparator() { return Long2ShortRBTreeMap.this.long2ShortEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final long k) { return in( k) && Long2ShortRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Long2ShortRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Long2ShortRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
