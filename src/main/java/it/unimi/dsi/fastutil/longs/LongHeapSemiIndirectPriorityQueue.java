...
public class LongHeapSemiIndirectPriorityQueue implements LongIndirectPriorityQueue {
	...
	public int dequeue() {
	 ...
	 heap[0] = heap[--size];
	 if (size != 0) LongSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
	 return result;
	}
	...
	@Override
	public void allChanged() { LongSemiIndirectHeaps.makeHeap(refArray, heap, size, c); }
	 ...
	 */
	@Override
	public int front(final int[] a) { return c == null ? LongSemiIndirectHeaps.front(refArray, heap, size, a) : LongSemiIndirectHeaps.front(refArray, heap, size, a, c); }
