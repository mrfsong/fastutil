...
public interface Long2ByteMap extends Long2ByteFunction , Map<Long,Byte> {
	...
	default byte replace(final long key, final byte value) { return containsKey(key) ? put(key, value) : defaultReturnValue(); }
	...
	default byte computeIfAbsentNullable(final long key, final java.util.function.LongFunction<? extends Byte> mappingFunction) {
	 java.util.Objects.requireNonNull(mappingFunction);
	 final byte v = get(key), drv = defaultReturnValue();
	 if (v != drv || containsKey(key)) return v;
	...
	default byte computeIfPresent(final long key, final java.util.function.BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
	...
	default byte compute(final long key, final java.util.function.BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final byte oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final long key, final byte value, final java.util.function.BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
