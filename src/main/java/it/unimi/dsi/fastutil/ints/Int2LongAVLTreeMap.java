...
public class Int2LongAVLTreeMap extends AbstractInt2LongSortedMap implements java.io.Serializable, Cloneable {
	...
	*/
	...
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Int2LongMap.Entry > {
	 ...
	 @Override
	 public Int2LongMap.Entry next() { return nextEntry(); }

	 @Override
	 public Int2LongMap.Entry previous() { return previousEntry(); }

	 @Override
	 public void set(Int2LongMap.Entry ok) { throw new UnsupportedOperationException(); }

	 @Override
	 public void add(Int2LongMap.Entry ok) { throw new UnsupportedOperationException(); }
	}


	@Override
	public ObjectSortedSet<Int2LongMap.Entry > int2LongEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Int2LongMap.Entry >() {
	   final Comparator<? super Int2LongMap.Entry > comparator = (Comparator<Int2LongMap.Entry >) (x, y) -> Int2LongAVLTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());

	   @Override
	   public Comparator<? super Int2LongMap.Entry > comparator() { return comparator; }

	   @Override
	   public ObjectBidirectionalIterator<Int2LongMap.Entry > iterator() { return new EntryIterator(); }

	   @Override
	   public ObjectBidirectionalIterator<Int2LongMap.Entry > iterator(final Int2LongMap.Entry from) { return new EntryIterator(from.getIntKey()); }
	   ...
	   @Override
	   public void clear() { Int2LongAVLTreeMap.this.clear(); }

	   @Override
	   public Int2LongMap.Entry first() { return firstEntry; }

	   @Override
	   public Int2LongMap.Entry last() { return lastEntry; }

	   @Override
	   public ObjectSortedSet<Int2LongMap.Entry > subSet(Int2LongMap.Entry from, Int2LongMap.Entry to) { return subMap(from.getIntKey(), to.getIntKey()).int2LongEntrySet(); }

	   @Override
	   public ObjectSortedSet<Int2LongMap.Entry > headSet(Int2LongMap.Entry to) { return headMap(to.getIntKey()).int2LongEntrySet(); }

	   @Override
	   public ObjectSortedSet<Int2LongMap.Entry > tailSet(Int2LongMap.Entry from) { return tailMap(from.getIntKey()).int2LongEntrySet(); }
	  };
	...
	@Override
	public Int2LongSortedMap headMap(int to) { return new Submap((0), true, to, false); }

	@Override
	public Int2LongSortedMap tailMap(int from) { return new Submap(from, false, (0), true); }

	@Override
	public Int2LongSortedMap subMap(int from, int to) { return new Submap(from, false, to, false); }
	...
	private final class Submap extends AbstractInt2LongSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final int from, final boolean bottom, final int to, final boolean top) {
	  if (! bottom && ! top && Int2LongAVLTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Int2LongMap.Entry > int2LongEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Int2LongMap.Entry >() {
	    @Override
	    public ObjectBidirectionalIterator<Int2LongMap.Entry > iterator() { return new SubmapEntryIterator(); }
	    @Override
	    public ObjectBidirectionalIterator<Int2LongMap.Entry > iterator(final Int2LongMap.Entry from) { return new SubmapEntryIterator(from.getIntKey()); }
	    @Override
	    public Comparator<? super Int2LongMap.Entry > comparator() { return Int2LongAVLTreeMap.this.int2LongEntrySet().comparator(); }
	    @Override
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Int2LongAVLTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Int2LongAVLTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
