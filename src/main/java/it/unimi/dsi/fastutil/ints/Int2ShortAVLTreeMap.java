...
public class Int2ShortAVLTreeMap extends AbstractInt2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	*/
	...
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Int2ShortMap.Entry > {
	 ...
	 @Override
	 public Int2ShortMap.Entry next() { return nextEntry(); }

	 @Override
	 public Int2ShortMap.Entry previous() { return previousEntry(); }

	 @Override
	 public void set(Int2ShortMap.Entry ok) { throw new UnsupportedOperationException(); }

	 @Override
	 public void add(Int2ShortMap.Entry ok) { throw new UnsupportedOperationException(); }
	}


	@Override
	public ObjectSortedSet<Int2ShortMap.Entry > int2ShortEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Int2ShortMap.Entry >() {
	   final Comparator<? super Int2ShortMap.Entry > comparator = (Comparator<Int2ShortMap.Entry >) (x, y) -> Int2ShortAVLTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());

	   @Override
	   public Comparator<? super Int2ShortMap.Entry > comparator() { return comparator; }

	   @Override
	   public ObjectBidirectionalIterator<Int2ShortMap.Entry > iterator() { return new EntryIterator(); }

	   @Override
	   public ObjectBidirectionalIterator<Int2ShortMap.Entry > iterator(final Int2ShortMap.Entry from) { return new EntryIterator(from.getIntKey()); }
	   ...
	   @Override
	   public void clear() { Int2ShortAVLTreeMap.this.clear(); }

	   @Override
	   public Int2ShortMap.Entry first() { return firstEntry; }

	   @Override
	   public Int2ShortMap.Entry last() { return lastEntry; }

	   @Override
	   public ObjectSortedSet<Int2ShortMap.Entry > subSet(Int2ShortMap.Entry from, Int2ShortMap.Entry to) { return subMap(from.getIntKey(), to.getIntKey()).int2ShortEntrySet(); }

	   @Override
	   public ObjectSortedSet<Int2ShortMap.Entry > headSet(Int2ShortMap.Entry to) { return headMap(to.getIntKey()).int2ShortEntrySet(); }

	   @Override
	   public ObjectSortedSet<Int2ShortMap.Entry > tailSet(Int2ShortMap.Entry from) { return tailMap(from.getIntKey()).int2ShortEntrySet(); }
	  };
	...
	@Override
	public Int2ShortSortedMap headMap(int to) { return new Submap((0), true, to, false); }

	@Override
	public Int2ShortSortedMap tailMap(int from) { return new Submap(from, false, (0), true); }

	@Override
	public Int2ShortSortedMap subMap(int from, int to) { return new Submap(from, false, to, false); }
	...
	private final class Submap extends AbstractInt2ShortSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final int from, final boolean bottom, final int to, final boolean top) {
	  if (! bottom && ! top && Int2ShortAVLTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Int2ShortMap.Entry > int2ShortEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Int2ShortMap.Entry >() {
	    @Override
	    public ObjectBidirectionalIterator<Int2ShortMap.Entry > iterator() { return new SubmapEntryIterator(); }
	    @Override
	    public ObjectBidirectionalIterator<Int2ShortMap.Entry > iterator(final Int2ShortMap.Entry from) { return new SubmapEntryIterator(from.getIntKey()); }
	    @Override
	    public Comparator<? super Int2ShortMap.Entry > comparator() { return Int2ShortAVLTreeMap.this.int2ShortEntrySet().comparator(); }
	    @Override
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Int2ShortAVLTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Int2ShortAVLTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
