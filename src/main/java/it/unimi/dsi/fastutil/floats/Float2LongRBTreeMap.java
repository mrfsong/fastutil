...
public class Float2LongRBTreeMap extends AbstractFloat2LongSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2LongMap.Entry > float2LongEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2LongMap.Entry >() {
	   final Comparator<? super Float2LongMap.Entry > comparator = (Comparator<Float2LongMap.Entry >) (x, y) -> Float2LongRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	   ...
	   @Override
	   public void clear() { Float2LongRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractFloat2LongSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2LongRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2LongMap.Entry > float2LongEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2LongMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Float2LongMap.Entry > comparator() { return Float2LongRBTreeMap.this.float2LongEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2LongRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2LongRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2LongRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
