...
public interface Float2ByteMap extends Float2ByteFunction , Map<Float,Byte> {
	...
	default byte replace(final float key, final byte value) { return containsKey(key) ? put(key, value) : defaultReturnValue(); }
	...
	default byte computeIfAbsentNullable(final float key, final java.util.function.DoubleFunction<? extends Byte> mappingFunction) {
	 java.util.Objects.requireNonNull(mappingFunction);
	 final byte v = get(key), drv = defaultReturnValue();
	 if (v != drv || containsKey(key)) return v;
	...
	default byte computeIfPresent(final float key, final java.util.function.BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
	...
	default byte compute(final float key, final java.util.function.BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final byte oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final float key, final byte value, final java.util.function.BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
