...
public class FloatBigArrayBigList extends AbstractFloatBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public FloatBigArrayBigList(final float a[][]) {
	 this(a, 0, FloatBigArrays.length(a));
	}
	...
	public FloatBigArrayBigList(final float a[][], final long offset, final long length) {
	 this(length);
	 FloatBigArrays.copy(a, offset, this.a, 0, length);
	 size = length;
	}
	...
	public static FloatBigArrayBigList wrap(final float a[][], final long length) {
	 if (length > FloatBigArrays.length(a)) throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size (" + FloatBigArrays.length(a) + ")");
	...
	public void ensureCapacity(final long capacity) {
	 if (capacity <= a.length || a == FloatBigArrays.DEFAULT_EMPTY_BIG_ARRAY) return;

	 a = FloatBigArrays.forceCapacity(a, capacity, size);
	 assert size <= FloatBigArrays.length(a);
	}
	...
	private void grow(long capacity) {
	 final long oldLength = FloatBigArrays.length(a);
	...
	public void add(final long index, final float k) {
	 ensureIndex(index);
	 grow(size + 1);
	 if (index != size) FloatBigArrays.copy(a, index, a, index + 1, size - index);
	 FloatBigArrays.set(a, index, k);
	 size++;
	 assert size <= FloatBigArrays.length(a);
	}
	...
	public long indexOf(final float k) {
	 for(long i = 0; i < size; i++) if (( Float.floatToIntBits(k) == Float.floatToIntBits(FloatBigArrays.get(a, i)) )) return i;
	 return -1;
	}

	@Override
	public long lastIndexOf(final float k) {
	 for(long i = size; i-- != 0;) if (( Float.floatToIntBits(k) == Float.floatToIntBits(FloatBigArrays.get(a, i)) )) return i;
	 return -1;
	}
	...
	public float removeFloat(final long index) {
	 if (index >= size) throw new IndexOutOfBoundsException("Index (" + index + ") is greater than or equal to list size (" + size + ")");
	 final float old = FloatBigArrays.get(a, index);
	 size--;
	 if (index != size) FloatBigArrays.copy(a, index + 1, a, index, size - index);
	...
	public float set(final long index, final float k) {
	 if (index >= size) throw new IndexOutOfBoundsException("Index (" + index + ") is greater than or equal to list size (" + size + ")");
	 float old = FloatBigArrays.get(a, index);
	 FloatBigArrays.set(a, index, k);
	 return old;
	}
	...
	public void size(final long size) {
	 if (size > FloatBigArrays.length(a)) a = FloatBigArrays.forceCapacity(a, size, this.size);
	 if (size > this.size) FloatBigArrays.fill(a, this.size, size, (0));
	...
	public void trim(final long n) {
	 final long arrayLength = FloatBigArrays.length(a);
	 if (n >= arrayLength || size == arrayLength) return;
	 a = FloatBigArrays.trim(a, Math.max(n, size));
	 assert size <= FloatBigArrays.length(a);
	}
	...
	public void removeElements(final long from, final long to) {
	 ...
	 FloatBigArrays.copy(a, to, a, from, size - to);
	 size -= (to - from);
	...
	public void addElements(final long index, final float a[][], final long offset, final long length) {
	 ...
	 grow(size + length);
	 FloatBigArrays.copy(this.a, index, this.a, index + length, size - index);
	...
	public FloatBigListIterator listIterator(final long index) {
	 ...
	 return new FloatBigListIterator () {
	   ...
	   @Override
	   public float nextFloat() { if (! hasNext()) throw new NoSuchElementException(); return FloatBigArrays.get(a, last = pos++); }
	   @Override
	   public float previousFloat() { if (! hasPrevious()) throw new NoSuchElementException(); return FloatBigArrays.get(a, last = --pos); }
	   @Override
	...
	public FloatBigArrayBigList clone() {
	 ...
	 FloatBigArrays.copy(a, 0, c.a, 0, size);
	 c.size = size;
	 return c;
	}
	...
	public boolean equals(final FloatBigArrayBigList l) {
	 ...
	 while(s-- != 0) if (FloatBigArrays.get(a1, s) != FloatBigArrays.get(a2, s)) return false;
	...
	public int compareTo(final FloatBigArrayBigList l) {
	 ...
	 for(i = 0; i < s1 && i < s2; i++) {
	  e1 = FloatBigArrays.get(a1, i);
	  e2 = FloatBigArrays.get(a2, i);
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
	 ...
	 for(int i = 0; i < size; i++) s.writeFloat(FloatBigArrays.get(a, i));
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
	 ...
	 for(int i = 0; i < size; i++) FloatBigArrays.set(a, i, s.readFloat());
	}
}
