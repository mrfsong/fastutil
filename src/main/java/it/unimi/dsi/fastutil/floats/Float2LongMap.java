...
public interface Float2LongMap extends Float2LongFunction , Map<Float,Long> {
	...
	default long replace(final float key, final long value) { return containsKey(key) ? put(key, value) : defaultReturnValue(); }
	...
	default long computeIfAbsentNullable(final float key, final java.util.function.DoubleFunction<? extends Long> mappingFunction) {
	 java.util.Objects.requireNonNull(mappingFunction);
	 final long v = get(key), drv = defaultReturnValue();
	 if (v != drv || containsKey(key)) return v;
	...
	default long computeIfPresent(final float key, final java.util.function.BiFunction<? super Float, ? super Long, ? extends Long> remappingFunction) {
	...
	default long compute(final float key, final java.util.function.BiFunction<? super Float, ? super Long, ? extends Long> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final long oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default long merge(final float key, final long value, final java.util.function.BiFunction<? super Long, ? super Long, ? extends Long> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
