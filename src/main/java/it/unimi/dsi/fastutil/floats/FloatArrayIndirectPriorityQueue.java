...
public class FloatArrayIndirectPriorityQueue implements FloatIndirectPriorityQueue {
	...
	private int findFirst() {
	 ...
	 if (c == null) while(i-- != 0) { if (( Float.compare((refArray[array[i]]),(first)) < 0 )) first = refArray[array[firstIndex = i]]; }
	...
	private int findLast() {
	 ...
	 if (c == null) { while(i-- != 0) if (( Float.compare((last),(refArray[array[i]])) < 0 )) last = refArray[array[lastIndex = i]]; }
	...
	public void enqueue(int x) {
	 ...
	 if (firstIndexValid) {
	  if (c == null) { if (( Float.compare((refArray[x]),(refArray[array[firstIndex]])) < 0 )) firstIndex = size; }
	...
	public int front(int[] a) {
	 ...
	 int i = size, c = 0;
	 while(i-- != 0) if (( Float.floatToIntBits(top) == Float.floatToIntBits(refArray[array[i]]) )) a[c++] = array[i];
	 return c;
	}
