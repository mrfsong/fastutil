...
public class Float2ShortRBTreeMap extends AbstractFloat2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2ShortMap.Entry > float2ShortEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2ShortMap.Entry >() {
	   final Comparator<? super Float2ShortMap.Entry > comparator = (Comparator<Float2ShortMap.Entry >) (x, y) -> Float2ShortRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	   ...
	   @Override
	   public void clear() { Float2ShortRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractFloat2ShortSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2ShortRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2ShortMap.Entry > float2ShortEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2ShortMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Float2ShortMap.Entry > comparator() { return Float2ShortRBTreeMap.this.float2ShortEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2ShortRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2ShortRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2ShortRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
