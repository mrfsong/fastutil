...
public class FloatOpenHashBigSet extends AbstractFloatSet implements java.io.Serializable, Cloneable, Hash, Size64 {
	...
	public boolean add(final float k) {
	 ...
	 if (( Float.floatToIntBits(k) == 0 )) {
	  ...
	  // The starting point.
	  if (! ( Float.floatToIntBits(curr = key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 )) {
	   if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return false;
	   while(! ( Float.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 ))
	    if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return false;
	  }
	 ...
	 for(;;) {
	  ...
	  for(;;) {
	   if (( Float.floatToIntBits(FloatBigArrays.get(key, pos)) == 0 )) {
	    FloatBigArrays.set(key, last, (0));
	    return;
	   }
	   slot = it.unimi.dsi.fastutil.HashCommon.mix( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(FloatBigArrays.get(key, pos)) ) ) & mask;
	...
	public boolean remove(final float k) {
	 ...
	 // The starting point.
	 if (( Float.floatToIntBits(curr = key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 )) return false;
	 if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return removeEntry(base, displ);
	 while(true) {
	  if (( Float.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 )) return false;
	  if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return removeEntry(base, displ);
	 }
	}
	...
	public boolean contains(final float k) {
	 ...
	 // The starting point.
	 if (( Float.floatToIntBits(curr = key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 )) return false;
	 if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return true;
	 while(true) {
	  if (( Float.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 )) return false;
	  if (( Float.floatToIntBits(curr) == Float.floatToIntBits(k) )) return true;
	 }
	...
	private class SetIterator implements FloatIterator {
	 ...
	 private final void shiftKeys(long pos) {
	  ...
	  for(;;) {
	   ...
	   for(;;) {
	    ...
	    slot = it.unimi.dsi.fastutil.HashCommon.mix( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(curr) ) ) & mask;
	    if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos) break;
	    pos = (pos + 1) & mask;
	   }
	   ...
	   if (pos < last) { // Wrapped entry.
	    ...
	    wrapped.add(FloatBigArrays.get(key, pos));
	   }
	 ...
	 public void remove() {
	  ...
	  if (last == n) FloatOpenHashBigSet.this.containsNull = false;
	...
	protected void rehash(final long newN) {
	 ...
	 for(long i = realSize(); i-- != 0;) {
	  ...
	  while(( Float.floatToIntBits(key[base][displ]) == 0 )) base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0));

	  k = key[base][displ];
	  h = it.unimi.dsi.fastutil.HashCommon.mix( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) );

	  // The starting point.
	  if (! ( Float.floatToIntBits(newKey[b = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][d = (int)(h & newSegmentMask)]) == 0 ))
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
	 ...
	 for(long j = size; j-- != 0;) s.writeFloat(i.nextFloat());
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
	 ...
	 for(long i = size; i-- != 0;) {
	  ...
	  if (( Float.floatToIntBits(k) == 0 )) containsNull = true;
	  else {
	   h = it.unimi.dsi.fastutil.HashCommon.mix( (long)( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) );
	   if (! ( Float.floatToIntBits(key[base = (int)((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int)(h & segmentMask)]) == 0 ))
	    while(! ( Float.floatToIntBits(key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0)) & baseMask][displ]) == 0 ));
	   key[base][displ] = k;
	  }
	 }
