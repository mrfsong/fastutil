...
public class Float2IntRBTreeMap extends AbstractFloat2IntSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2IntMap.Entry > float2IntEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2IntMap.Entry >() {
	   final Comparator<? super Float2IntMap.Entry > comparator = (Comparator<Float2IntMap.Entry >) (x, y) -> Float2IntRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	   ...
	   @Override
	   public void clear() { Float2IntRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractFloat2IntSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2IntRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2IntMap.Entry > float2IntEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2IntMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Float2IntMap.Entry > comparator() { return Float2IntRBTreeMap.this.float2IntEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2IntRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2IntRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2IntRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
