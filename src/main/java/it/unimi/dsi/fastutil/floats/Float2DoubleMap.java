...
public interface Float2DoubleMap extends Float2DoubleFunction , Map<Float,Double> {
	...
	default double replace(final float key, final double value) { return containsKey(key) ? put(key, value) : defaultReturnValue(); }
	...
	default double computeIfAbsentNullable(final float key, final java.util.function.DoubleFunction<? extends Double> mappingFunction) {
	 java.util.Objects.requireNonNull(mappingFunction);
	 final double v = get(key), drv = defaultReturnValue();
	 if (v != drv || containsKey(key)) return v;
	...
	default double computeIfPresent(final float key, final java.util.function.BiFunction<? super Float, ? super Double, ? extends Double> remappingFunction) {
	...
	default double compute(final float key, final java.util.function.BiFunction<? super Float, ? super Double, ? extends Double> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
	 final double oldValue = get(key), drv = defaultReturnValue();
	 final boolean contained = oldValue != drv || containsKey(key);
	 final Double newValue = remappingFunction.apply(Float.valueOf(key), contained ? Double.valueOf(oldValue) : null);
	 ...
	 * @since 8.0.0
	 */
	default double merge(final float key, final double value, final java.util.function.BiFunction<? super Double, ? super Double, ? extends Double> remappingFunction) {
	 java.util.Objects.requireNonNull(remappingFunction);
