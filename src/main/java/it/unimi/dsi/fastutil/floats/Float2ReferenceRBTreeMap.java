...
public class Float2ReferenceRBTreeMap <V> extends AbstractFloat2ReferenceSortedMap <V> implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2ReferenceMap.Entry <V> > float2ReferenceEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry <V> >() {
	   final Comparator<? super Float2ReferenceMap.Entry <V> > comparator = (Comparator<Float2ReferenceMap.Entry <V> >) (x, y) -> Float2ReferenceRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	   ...
	   @Override
	   public void clear() { Float2ReferenceRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractFloat2ReferenceSortedMap <V> implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2ReferenceRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2ReferenceMap.Entry <V> > float2ReferenceEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry <V> >() {
	    ...
	    @Override
	    public Comparator<? super Float2ReferenceMap.Entry <V> > comparator() { return Float2ReferenceRBTreeMap.this.float2ReferenceEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2ReferenceRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2ReferenceRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2ReferenceRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
