...
public class Float2BooleanRBTreeMap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2BooleanMap.Entry > float2BooleanEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2BooleanMap.Entry >() {
	   final Comparator<? super Float2BooleanMap.Entry > comparator = (Comparator<Float2BooleanMap.Entry >) (x, y) -> Float2BooleanRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());

	   @Override
	   public Comparator<? super Float2BooleanMap.Entry > comparator() { return comparator; }

	   @Override
	   public ObjectBidirectionalIterator<Float2BooleanMap.Entry > iterator() { return new EntryIterator(); }

	   @Override
	   public ObjectBidirectionalIterator<Float2BooleanMap.Entry > iterator(final Float2BooleanMap.Entry from) { return new EntryIterator(from.getFloatKey()); }
	   ...
	   @Override
	   public void clear() { Float2BooleanRBTreeMap.this.clear(); }
	...
	@Override
	public Float2BooleanSortedMap headMap(float to) { return new Submap((0), true, to, false); }

	@Override
	public Float2BooleanSortedMap tailMap(float from) { return new Submap(from, false, (0), true); }

	@Override
	public Float2BooleanSortedMap subMap(float from, float to) { return new Submap(from, false, to, false); }
	...
	private final class Submap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2BooleanRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2BooleanMap.Entry > float2BooleanEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2BooleanMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Float2BooleanMap.Entry > comparator() { return Float2BooleanRBTreeMap.this.float2BooleanEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2BooleanRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2BooleanRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2BooleanRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
