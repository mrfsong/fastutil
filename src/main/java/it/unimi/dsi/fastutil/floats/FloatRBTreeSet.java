...
public class FloatRBTreeSet extends AbstractFloatSortedSet implements java.io.Serializable, Cloneable, FloatSortedSet {
	...
	*/
	...
	@Override
	public FloatBidirectionalIterator iterator() { return new SetIterator(); }

	@Override
	public FloatBidirectionalIterator iterator(final float from) { return new SetIterator(from); }

	@Override
	public FloatComparator comparator() { return actualComparator; }

	@Override
	public FloatSortedSet headSet(final float to) { return new Subset((0), true, to, false); }

	@Override
	public FloatSortedSet tailSet(final float from) { return new Subset(from, false, (0), true); }

	@Override
	public FloatSortedSet subSet(final float from, final float to) { return new Subset(from, false, to, false); }
	...
	private final class Subset extends AbstractFloatSortedSet implements java.io.Serializable, FloatSortedSet {
	 ...
	 public Subset(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && FloatRBTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")");
	 ...
	 private final class SubsetIterator extends SetIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && FloatRBTreeSet.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && FloatRBTreeSet.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
	 ...
	 while(n-- != 0) s.writeFloat(i.nextFloat());
	}
