...
public class Float2CharRBTreeMap extends AbstractFloat2CharSortedMap implements java.io.Serializable, Cloneable {
	...
	public ObjectSortedSet<Float2CharMap.Entry > float2CharEntrySet() {
	 if (entries == null) entries = new AbstractObjectSortedSet<Float2CharMap.Entry >() {
	   final Comparator<? super Float2CharMap.Entry > comparator = (Comparator<Float2CharMap.Entry >) (x, y) -> Float2CharRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	   ...
	   @Override
	   public void clear() { Float2CharRBTreeMap.this.clear(); }
	...
	private final class Submap extends AbstractFloat2CharSortedMap implements java.io.Serializable {
	 ...
	 public Submap(final float from, final boolean bottom, final float to, final boolean top) {
	  if (! bottom && ! top && Float2CharRBTreeMap.this.compare(from, to) > 0) throw new IllegalArgumentException("Start key (" + from + ") is larger than end key (" + to + ")");
	 ...
	 public ObjectSortedSet<Float2CharMap.Entry > float2CharEntrySet() {
	  if (entries == null) entries = new AbstractObjectSortedSet<Float2CharMap.Entry >() {
	    ...
	    @Override
	    public Comparator<? super Float2CharMap.Entry > comparator() { return Float2CharRBTreeMap.this.float2CharEntrySet().comparator(); }
	 ...
	 public boolean containsKey(final float k) { return in( k) && Float2CharRBTreeMap.this.containsKey(k); }
	 ...
	 private class SubmapIterator extends TreeIterator {
	  ...
	  void updatePrevious() {
	   prev = prev.prev();
	   if (! bottom && prev != null && Float2CharRBTreeMap.this.compare(prev.key, from) < 0) prev = null;
	  }

	  @Override
	  void updateNext() {
	   next = next.next();
	   if (! top && next != null && Float2CharRBTreeMap.this.compare(next.key, to) >= 0) next = null;
	  }
	 }
